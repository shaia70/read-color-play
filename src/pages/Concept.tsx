            ...
            <div className="glass-card p-8 flex flex-col items-center">
              ...
              <ul className="w-full space-y-3 mb-6">
                <li className="flex items-start">
                  ...
                  <span className={language === 'en' ? 'text-center' : ''}>{language === 'en' ? 'Fascinating and enriching stories' : 'סיפורים מרתקים ומעשירים'}</span>
                </li>
                <li className="flex items-start">
                  <ArrowRight className={`w-5 h-5 text-shelley-blue ${language === 'en' ? 'mr-2' : 'ml-2'} mt-1 flex-shrink-0`} />
                  <span className={language === 'en' ? 'text-center' : ''}>{language === 'en' ? 'Original and stunning illustrations' : 'איורים מקוריים ומרהיבים'}</span>
                </li>
                <li className="flex items-start">
                  <ArrowRight className={`w-5 h-5 text-shelley-blue ${language === 'en' ? 'mr-2' : 'ml-2'} mt-1 flex-shrink-0`} />
                  <span className={language === 'en' ? 'text-center' : ''}>{language === 'en' ? 'Quality hardcover' : 'כריכה קשה ואיכותית'}</span>
                </li>
                <li className="flex items-start">
                  <ArrowRight className={`w-5 h-5 text-shelley-blue ${language === 'en' ? 'mr-2' : 'ml-2'} mt-1 flex-shrink-0`} />
                  <span className={language === 'en' ? 'text-center' : ''}>{language === 'en' ? 'Thick and durable paper' : 'נייר עבה ועמיד'}</span>
                </li>
              </ul>
